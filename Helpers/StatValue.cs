...
namespace AI_Research_1.Helpers
{
    public class StatValue
    {
        ...
        public override string ToString() => $"{FormatCompact(Mean)} ± {FormatCompact(ConfIntervalSize/2)}";
        ...
        public string ToDetailedString(bool humanReadable = true)
        {
            if (humanReadable)
                return $"{Mean} " +
                       $"disp={StdDeviation} " +
                       $"min..max={Min}..{Max} " +
                       $"confInt={ConfIntervalSize} " +
                       $"count={Count}";
            return $"{Mean}\t{StdDeviation}\t{ConfIntervalSize}\t{Count}";
        }
